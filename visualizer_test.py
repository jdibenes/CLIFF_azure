...
class demo:
    def __init__(self, vertices, faces, joints, smpl_segment):
        ...
        self._vsv = visualizer.vsv(np.pi / 3.0, 1280, 720)
        self._vsv.set_smpl_mesh(vertices, faces, joints, smpl_segment)

        self._default_colors = self._vsv.get_mesh_colors().copy() #default color = [102 102 102 255] uint8
        self._focus_regions = [
            self._vsv.focus_left_foot,
            self._vsv.focus_right_foot,
            self._vsv.focus_left_lower_leg,
            self._vsv.focus_right_lower_leg,
            self._vsv.focus_left_thigh,
            self._vsv.focus_right_thigh,
            self._vsv.focus_center_body,
            self._vsv.focus_center_head,
            self._vsv.focus_left_upper_arm,
            self._vsv.focus_right_upper_arm,
        ]
    ...
    def update_surface_position(self):
    ...
    def traverse(self, viewer, delta_displacement, delta_angle):
        self._position = [self._position[0] + delta_displacement, self._position[1] + delta_angle]
        self.update_surface_position()
...
vertices = np.load('test_vertices.npy')
faces = np.load('test_faces.npy')
joints = np.load('test_joints.npy')
with open('./smpl_vert_segmentation.json') as file:
